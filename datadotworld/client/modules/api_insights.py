...
class InsightsApi(object):
	...
	def create_insight(self, owner_id, project_id, **kwargs):
        ...
        '''
		try:
			url = '{}/datasets/{}/{}'.format(self.api_client._api_url, owner_id, project_id)
	...
	def update_insight(self, owner_id, dataset_id, **kwargs):
		'''Update an existing dataset. Only elements included in the request will be updated. All omitted elements will remain untouched.
		:param owner_id: User or organization ID of the owner of the dataset
        :type owner_id: str
        ...
        '''
		try:
			url = '{}/datasets/{}/{}'.format(self.api_client._api_url, owner_id, dataset_id)
			r = requests.patch(url, headers=self.api_client.default_headers, data=json.dumps(kwargs))
		except RequestException as e:
			raise e

	def replace_insight(self, owner_id, dataset_id, **kwargs):
