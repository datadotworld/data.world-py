...
class StreamsApi(object):
    ...
    def append_records(self, owner, id, stream_id, body, **kwargs):
        ...
        This endpoint appends JSON data to a data stream associated with a dataset.  Multiple records can be appended at once by using JSON-L (`application/json-l`) as the request content type.  Currently, data uploaded to a dataset via a stream is not immediatelly processed. Instead, it is processed automatically a few times per day or as a result of a \"Sync files\" endpoint invocation.  Once processed, the contents of a stream will appear as part of the respective dataset as a `.jsonl` file (e.g. `my-stream` will produce a file named `my-stream.jsonl`).
        ...
        :param str stream_id: Stream unique identifier. (required)
