...
class Config(object):
    ...
    def __init__(self, profile='default', **kwargs):
        ...
        config_file_path = path.expanduser(kwargs.get('config_file_path', '~/.dw/config'))
        legacy_file_path = path.expanduser(kwargs.get('legacy_file_path', '~/.data.world'))

        if not path.isdir(path.dirname(config_file_path)):
            os.makedirs(path.dirname(config_file_path))

        config_parser = configparser.ConfigParser() if six.PY3 else configparser.SafeConfigParser()
