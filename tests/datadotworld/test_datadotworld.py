...
class TestDataDotWorld:
    ...
    def existing_dataset(self, config, test_files_path):
        ...
        dest_dir = path.join(config.cache_dir, 'agentid', 'datasetid', 'latest')
        shutil.copytree(path.join(test_files_path, 'the-simpsons-by-the-data-existing'), dest_dir)

    @pytest.fixture()
    def api_client(self, test_files_path):
        with Spy(RestApiClient) as client:
            def download_datapackage(_, dest_dir):
                shutil.copytree(path.join(test_files_path, 'the-simpsons-by-the-data'), dest_dir)
                return path.join(dest_dir, 'datapackage.json')

            client.download_datapackage(anything(), anything()).delegates(download_datapackage)
            return client
    ...
    query_types = [
        ...
        ('sparql', 'https://query.data.world/sparql/agentid/datasetid', 'notreallysparql'),
        ('sql', 'https://query.data.world/sql/agentid/datasetid', 'notreallysql')
    ]

    @pytest.mark.parametrize("type,endpoint,query", query_types, ids=['sparql', 'sql'])
    def test_query(self, helpers, dw, dataset_key, type, endpoint, query, query_result_csv):
    ...
    def test_query_400(self, dw, dataset_key, type, endpoint, query):
        with responses.RequestsMock() as rsps:
            ...
            assert_that(calling(dw.query).with_args(dataset_key, query, query_type=type), raises(RuntimeError))
    ...
    def test_load_dataset(self, config, api_client, dw, dataset_key):
        ...
        assert_that(api_client.download_datapackage,
                    called().times(1).with_args(equal_to(dataset_key), equal_to(dest_dir)))
    ...
    def test_load_dataset_forced_fallback(self, api_client, dw, dataset_key):
        when(api_client).download_datapackage(equal_to(dataset_key), anything()).raises(RestApiError)
...
def dw_instances(monkeypatch):
    ...
    with Spy(DataDotWorld) as dw, Spy(DataDotWorld) as dw_alternative:
        ...
        monkeypatch.setattr(datadotworld, '_get_instance',
                            lambda profile: dw if profile == 'default' else dw_alternative)
        return {'default': dw, 'alternative': dw_alternative}


@pytest.fixture(params=['default', 'alternative'], ids=['default', 'alternative'])
def profile(request):
...
def test_toplevel_load_dataset(dw_instances, profile):
    ...
    assert_that(dw_instances[profile].load_dataset,
                called().times(1).with_args(equal_to('agentid/datasetid'), force_update=equal_to(False)))


def test_toplevel_query(dw_instances, profile):
    datadotworld.query('agentid/datasetid', 'SELECT * FROM Tables', profile=profile)
    assert_that(dw_instances[profile].query,
                called().times(1).with_args(equal_to('agentid/datasetid'), equal_to('SELECT * FROM Tables'),
                                            query_type=equal_to('sql')))


def test_toplevel_api_client(dw_instances, profile):
    assert_that(datadotworld.api_client(), equal_to(dw_instances[profile].api_client))
