...
class TestLocalDataset:
    ...
    def simpsons_descriptor_path(self, test_files_path):
        return path.join(test_files_path, 'the-simpsons-by-the-data', 'datapackage.json')
    ...
    def test_describe(self, simpsons_dataset, simpsons_datapackage):
        assert_that(simpsons_dataset.describe(), equal_to(simpsons_datapackage.descriptor))

    def test_describe_resource(self, simpsons_dataset, simpsons_datapackage):
        for r in simpsons_datapackage.resources:
            assert_that(simpsons_dataset.describe(r.descriptor['name']), equal_to(r.descriptor))

    def test_raw_data(self, simpsons_dataset, simpsons_datapackage, simpsons_descriptor_path):
        for r in simpsons_datapackage.resources:
            resource = Resource(r.descriptor, default_base_path=path.dirname(simpsons_descriptor_path))
            assert_that(simpsons_dataset.raw_data[r.descriptor['name']], equal_to(resource.data))

    def test_tables(self, simpsons_dataset, simpsons_datapackage):
        for r in simpsons_datapackage.resources:
            if r.descriptor['name'] in simpsons_dataset.tables:
                assert_that(simpsons_dataset.tables[r.descriptor['name']], equal_to(r.data))
