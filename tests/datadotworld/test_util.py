...
class TestLazyLoadedDict:
    ...
    def test_repr(self, lazy_dict):
        assert_that(repr(lazy_dict), equal_to('<datadotworld.util.LazyLoadedDict with values of type: str>'))
    ...
    def test_str_loaded(self, lazy_dict, dict_keys, loader_fn):
        ...
        assert_that(str(lazy_dict),
                    equal_to("{{'key1': {0}, 'key2': {1}}}".format(*expected_values)))

    def test_str_unloaded(self, lazy_dict):
        assert_that(str(lazy_dict), equal_to("{{'key1': {0}, 'key2': {0}}}".format('<str>')))
